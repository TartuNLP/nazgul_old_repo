...
## Mis päringu/vastuse sisu formaat? (Selle mõtleme koos välja, aga mis praegu on?)
## Päringuks ja vastuseks peaks olema utf-8 encoded string. 
## Vajadusel võin json-iga arvestada mõlemal juhul.
##

## Kas võib/tuleb kasutada erinevaid ühendusi erinevate päringute jaoks?
## One connection has one request handled at time. At the moment I think it closes connection
## after the request is fulfilled.

## Mitu päringut suudab see samal ajal täita?
## Answer: n + 1, handled 20 simultaneous well in my opinion, haven't tried more.
import argparse
import codecs
import json
import logging
...
def translate_model(queue, rqueue, pid, models, options, k, normalize, verbose, nbest,
                    return_alignment, suppress_unk, return_hyp_graph, fs_init, fs_next):
...
def main(models, saveto, bpe_file, save_alignment=None, k=5,
         normalize=False, n_process=5, chr_level=False, verbose=False, nbest=False,
         suppress_unk=False, a_json=False, print_word_probabilities=False,
        ...
        while source_file_t[0] != "EOT":
            for i in range(len(source_file_t)):
                ...
                source_file_t[i] = bpe.segment(
                    tokenizer.tokenize(source_file_t[i], return_str=True)).strip()
            #print "Passed"
            print source_file_t
            detokenized = ''
            queue = Queue()
            ...
            for midx in xrange(n_process):
                processes[midx] = Process(
                    ...
                    args=(
                    queue, rqueue, midx, models, options, k, normalize, verbose, nbest, save_alignment is not None,
                    suppress_unk, return_hyp_graph, fs_init, fs_next))
                processes[midx].start()

            n_samples, source_sentences = _send_jobs(source_file_t, processes, queue)
            _finish_processes(queue)
            #### The model loading takes place in the head of for loop, prolly in _retrieve_jobs
            for i, trans in enumerate(_retrieve_jobs(n_samples, processes, queue, rqueue)):
                print "NEXT SENTENCE:"
                if nbest:
                    ...
                    for j in order:
                        ...
                        if save_alignment is not None:
                            if a_json:
                                print_matrix_json(alignment[j], source_sentences[i], _seqs2words(samples[j]).split(),
                                              i, i+j,save_alignment)
                            else:
                    ...
                    #print x[0].upper() + x[1:]
                    detokenized += detokenizer.detokenize((x.decode('utf-8') + " ").split(), return_str=True)
                    ...
                    if save_alignment is not None:
                        if a_json:
                            print_matrix_json(alignment, source_sentences[i], _seqs2words(trans[0]).split(),
                                          i, i,save_alignment)
                        else:
                            save_alignment.write('{0} ||| {1} ||| {2} ||| {3} ||| {4} {5}\n'.format(
                                              i, _seqs2words(trans[0]), 0, ' '.join(source_sentences[i]) ,
            ...
            try:  # Establish connection with client.
                ...
                try:
                    t = threading.Thread(target=_parallelized_main,
                                         args=(fs_init, fs_next, c, bpe, tokenizer, detokenizer))
                    t.start()
                    t.join()
                except socket.error:
                    c.close()
    ...
    for model, option in zip(models, options):
        ...
        f_init, f_next = build_sampler(tparams, option, use_noise, trng,
                                       return_alignment=save_alignment is not None)
    ...
    while True:
        try:
            ...
            t = threading.Thread(target=_listen,
                                 args=(c, addr, fs_init, fs_next, tokenizer, detokenizer, bpe))
            t.start()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bpe', '-b', type=str, nargs='+', required=True,
    ...
    main(args.models,
         args.output, args.bpe, k=args.k, normalize=args.n, n_process=args.p,
         ...
         a_json=args.json_alignment, return_hyp_graph=args.search_graph)
